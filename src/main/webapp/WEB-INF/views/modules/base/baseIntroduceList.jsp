...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/base/baseIntroduce/">介绍列表</a></li>
		...
		<ul class="ul-form">
			<li><label>创建时间：</label>
				<input name="beginCreateDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${baseIntroduce.beginCreateDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/> - 
				<input name="endCreateDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${baseIntroduce.endCreateDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</li>
			<li><label>标题：</label>
		...
		<c:forEach items="${page.list}" var="baseIntroduce">
			<tr>
				<td><a href="${ctx}/base/baseIntroduce/form?id=${baseIntroduce.id}">
					<fmt:formatDate value="${baseIntroduce.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</a></td>
				<td>
